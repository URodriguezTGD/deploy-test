...
const article = ({ article, articles }) => {
    ...
    return (
        <>
            <Head >
                <title>BrainTax | {article.title}</title>
            </Head>
            <div key={article.id} className={`${PostStyles.container}`}>
                {postImageUrl && (
                    <img src={postImageUrl} className={PostStyles.mainImage} />
                )}
                <h1>{article.title}</h1>
                <div className={`${PostStyles.published} section`}>
                    ...
                    <div>
                        <p>
                            Publicado el{" "}
                            {format(new Date(article.createdAt), "dd")} de{" "}
                            {translateMonth(
                                format(new Date(article.createdAt), "M")
                            )}{" "}
                            del {format(new Date(article.createdAt), "yyyy")}
                        </p>
...
export const getStaticPaths = async (c) => {
    ...
    const paths = slugs.map((slug) => ({
        params: { id: slug.slug },
        locale: slug.language === "en_US" ? "en" : "es",
    }));
    return {
        paths,
        fallback: false,
